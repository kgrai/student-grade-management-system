...
if (((branch = "cs") || (branch = "ec") || (branch = "me") || (branch = "cv")) && ((sem == "1st Semester") || (sem == "2nd Semester")) && scheme == "18") {
    ...
    function calc() {
        ...
        document.getElementById("res").value = document.getElementById("SGPAresult").innerHTML = result.toFixed(2);
...
//17 scheme
    ...
    window.onload = function() {
        ...
        if (sem == "1st Semester") {
            ...
            document.getElementById("ct1").textContent = "17**";
...
function calccgpa() {
    ...
    var credSum = 0;
    if (c[0] > 0) { credSum += 24; } else { c[0] = 0; }
    if (c[1] > 0) { credSum += 24; } else { c[1] = 0; }
    if (c[2] > 0) { credSum += 28; } else { c[2] = 0; }
    if (c[3] > 0) { credSum += 28; } else { c[3] = 0; }
    if (c[4] > 0) { credSum += 26; } else { c[4] = 0; }
    if (c[5] > 0) { credSum += 26; } else { c[5] = 0; }
    if (c[6] > 0) { credSum += 24; } else { c[6] = 0; }
    if (c[7] > 0) { credSum += 20; } else { c[7] = 0; }
    for (var i = 0; i < 8; i++) {
        console.log(c[i]);
    }
